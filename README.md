...
**main_training.py** creates a folder under named **experiment_name** (given as argument **--en**) under **result_files** folder. Two files are created under **results_files/<experiment_name>**: One of them starts with **predictions_** which contains predictions for independent test dataset. The other one starts with the **performance_results** which contains the best performance results for each fold (if setting-1 is chosen) or for test dataset (if setting-2 is chosen).
